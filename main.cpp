...
public:
	...
	LinAlgType linalg;
	AlgorithmType alg;	// ? ist das hier default-intialized als AlgorithmType::none ?
	...
	void run() {
		...
		if (alg == AlgorithmType::colStochJLT || alg == AlgorithmType::colStoch || alg == AlgorithmType::specStoch) {
		...
		if (alg == AlgorithmType::simplStochJLT || alg == AlgorithmType::simplStoch || alg == AlgorithmType::colStoch || alg == AlgorithmType::colStochJLT || alg == AlgorithmType::specStoch) {
...
int main(int argc, char* argv[])
{
  ...
  ierr = PetscInitialize(&argc,&argv,(char*)0, "START\n"); if(ierr) {return ierr;}
  {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (arg == "-a1") { experiment.alg = AlgorithmType::stGreedy; continue; }
		if (arg == "-a2") { experiment.alg = AlgorithmType::simplStoch; continue; }
		if (arg == "-a3") { experiment.alg = AlgorithmType::colStoch; continue; }
		if (arg == "-a5") { experiment.alg = AlgorithmType::simplStochJLT; continue; }
		if (arg == "-a6") { experiment.alg = AlgorithmType::colStochJLT; continue; }
		if (arg == "-a7") { experiment.alg = AlgorithmType::specStoch; continue; }
		...
		if (arg == "--lamg") 	 { linalg = LinAlgType::lamg; }
		if (arg == "--lu") 		 { linalg = LinAlgType::lu; }
		if (arg == "--jlt-lu") 	 { linalg = LinAlgType::jlt_lu_sparse; }
		if (arg == "--jlt-lamg") { linalg = LinAlgType::jlt_lamg; }
